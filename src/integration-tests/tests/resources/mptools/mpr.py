...
def find_serial_device(dev):
    print("find_serial_device(" + dev + ")")
    if dev is None:
        for d in device_shortcuts.values():
            try:
                os.stat(d)
            except OSError:
                continue
        ...
        dev_name = dev
        try:
            dev_name = device_shortcuts.get(dev, dev)
        except Exception:
            pass
        try:
            os.fspath(dev_name)
        except OSError:
            print(f"{dev_name} could not be accessed")
            sys.exit(1)
        return dev
...
class PyboardExtended(pyboard.Pyboard):
    ...
    def enter_raw_repl_without_soft_reset(self):
        ...
        if not data.endswith(b"raw REPL; CTRL-B to exit\r\n"):
            raise pyboard.PyboardError("could not enter raw repl")
...
def main():
    ...
    try:
        while args:
            ...
            cmds = {
                # command: (need_raw_repl,   is_action,  min_args_reqd
                "mount": (True, False, 1),
                "repl": (False, True, 0),
                "eval": (True, True, 1),
                "exec": (True, True, 1),
